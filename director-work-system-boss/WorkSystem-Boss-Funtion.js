...
//2023/01/30 ++
//主管幫員工補打卡
//格式 : function+方法名稱(可自訂)+(你這個方法要帶入的參數) 
function forgotToPunchCard(managerEmployeeCode, employeeCode, punchWorkTime, offWorkTime) {
    //容器 = {key:value} ps.基本上這裡是看你後端的api要帶入什麼參數就有幾對key/value ps.key是req的名子value對照上面
    let workData = { employeeCode: employeeCode, managerEmployeeCode: managerEmployeeCode, punchWorkTime: punchWorkTime, offWorkTime: offWorkTime }
    //jquery語法
    $.ajax({
        url: 'http://localhost:8080/api/forgotToPunchCard',
        method: 'POST',
        contentType: 'application/json',
        dataType: 'json',
        //將你上面的容器名稱帶入
        data: JSON.stringify(workData),
        //成功進去這個判斷，並且看你需要做什麼事情 格式: success: function (後端res名稱) 
        success: function (WorkSystemRes) {
            //以下都是要做ㄉ事情
            $('#employeeCode').empty()
            $('#workTime').empty()
            ...
            $('#attendanceStatus').empty()
            $('#attendanceHours').empty()
            //在這個方法裡面要回應給用戶的東西 =  後端res ps.回應的東西是對照res裡面的參數名稱
            
            let { message, workSystem } = WorkSystemRes
            // let workSystem=WorkSystemRes
            if (message != null) {
                alert(message)
            }
            let workTime = (workSystem.workTime).replace('T', ' ');
